...
def read_fields_data_in_db():
    ...
    with get_session() as session:  # Establishing a database session
        ...
        # If file status is 'BRONZE_PROCESSED', move to silver validation
        if results.file_status == 'SILVER_PROCESSED':
            logger.info(f"Updating file '{results.filename}' status to 'GOLD_PROCESSING'.")
            update_file_status(session, 'GOLD_PROCESSING', results.id)  # Updating status
            file_processor.ingest()  # Processing file
            logger.info(f"Field gold processing completed successfully. Updating file '{results.filename}' status to 'GOLD_PROCESSED'.")
            update_file_status(session, 'GOLD_PROCESSED', results.id)  # Updating to final processed status
            return
